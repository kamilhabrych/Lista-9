    ...
    for i in range((2 * radius)+1):
        for j in range((2 * radius)+1):
            ...
            if (dist > radius - 0.5 and dist < radius + 0.5):
                listOfPoints.append(Point(point.y+radius-
j,point.x+radius-i))
notFound = True
listOfPoints = []
p = Point(0,0)
...
while 1:
    ...
    if p.x+radius <= 50 and p.x-
radius >= 1 and p.y+radius <= 50 and p.y-radius >= 1:
break
printPattern(radius,p)
with open('okrag.txt','w') as fo:
        ...
        for column in range(1,51):
            ...
            for obj in listOfPoints:
                if (column == obj.x and row == obj.y):
                    fo.write("*")
                    notFound = False
            if notFound == True: fo.write(" ")
        fo.write("\n")
    fo.write("Okrag o punkcie centrum p=({0},{1}) i promieniu
{2}".format(p.x,p.y,radius))
    fo.close()